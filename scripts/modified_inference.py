...
# ========== Settings ==========
upscale = 4
mode_of_operation = int(input(f'Enter 1 if train data is used else enter 0:'))
if mode_of_operation == 0:
    lr_path = "/scratch/dshajkum/Image_resolution/new_test/"
    model_path = "model_weights_SWIN_optimised.pth"  # Change if saved differently
    num_samples_to_show = 3
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    assert os.path.exists(lr_path), f"LR path does not exist: {lr_path}"    
    
    # ========== Load Dataset ==========
    dataset = SRDataset_modified(lr_path)
    ...
    os.makedirs("results_800epochs", exist_ok=True)    
    for i, lr in enumerate(loader):
        lr = lr.to(device)    
        with torch.no_grad():
            sr = model(lr)    
        ...
        plt.tight_layout()
        plt.savefig(f"results_800epochs/sample_{i}.png")
else:
    psnr_list, ssim_list = [], []
    # ========== Settings ==========
    upscale = 4
    lr_path = "/scratch/dshajkum/Image_resolution/test_data/DIV2K_train_LR_bicubic/X4"
    hr_path = "/scratch/dshajkum/Image_resolution/test_data/DIV2K_train_HR"
    model_path = "model_weights_SWIN_100.pth"  # Change if saved differently
    ...
    # ========== Inference and Visualization ==========
    os.makedirs("results_100_epochs", exist_ok=True)
    ...
    for i, (lr, hr) in enumerate(loader):
        ...
        plt.tight_layout()
        plt.savefig(f"results_100_epochs/sample_{i}.png")
        #plt.show()
