...
def build_model(mc: dict, device):
    ...
    print(f" Loading weights from {mc['model_path']}")
    model.load_state_dict(torch.load(mc["model_path"], map_location=device))
    model.eval()
    print(" Model ready")
    return model
...
def main():
    ...
    if args.test:
        ...
        out_dir  = root / cfg["inference"]["results_dir_test"]
        ...
        out_dir  = root / cfg["inference"]["results_dir_train"]
